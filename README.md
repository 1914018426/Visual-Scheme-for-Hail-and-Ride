...
基于 YOLO11-Pose 与 MediaPipe Hands 的实时手势识别检测系统，支持"招手"手势的精确识别与方向决策。
...
| 手势识别 | 支持 `waving`（招手）、`hand_up`（举手）两种手势输出，内部保留方向区分用于调试 |
...
- `θ1 > 25°` → 手臂抬起 → 候选 **waving（招手）**
- `15° < θ1 < 150°` → 手臂平伸或高举 → 候选 **waving（招手）**
...
| 条件 | 结果 | 说明 |
|------|------|------|
| 垂直/水平/对角线挥动 + 手臂抬起 | **waving** | 招手：各种方向的挥手动作 |
| 垂直挥动为主 (>55%) + 手臂高举 | **waving** | 高举手臂上下挥 |
| 水平挥动为主 (>55%) + 手臂平伸 | **waving** | 平伸手臂左右挥 |
